 ...
 * 
 * @author Thach.Le, truongtho88.nl
 */
...
public class ExRequestJpaController extends RequestJpaController {
    ...
    * @see mks.dms.dao.controller.RequestJpaController#findRequestEntities()
    */
    public List<Request> findRequestEntities(String groupId) {
        ...
        try {
            ...
            // Add filter request within group
            Predicate predicateGroupId = cb.equal(rootReq.get("groupId"), groupId);

            cq.where(predicateGroupId);
...
//    	System.out.println("requestTypeCd: " + requestTypeCd);
    ...
    public List<Request> findRequestByType(String requestTypeCd, String groupName) {
        if (groupName != null) {
            EntityManager em = getEntityManager();
            try {
                Query query = em
                        .createQuery("Select r FROM Request r WHERE (r.requesttypeCd = :requesttypeCd) and (r.groupId = :groupId)");
                query.setParameter("requesttypeCd", requestTypeCd);
                query.setParameter("groupId", groupName);
                List<Request> lstRequest = (List<Request>) query.getResultList();
            ...
            return findRequestByType(requestTypeCd);
        }
    }
    ...
    */
    public List<Request> findRequestOfUserByCondition(String username, SearchRequestConditionModel searchCond, String groupId) {
        List<Request> lstRequest;
        EntityManager em = getEntityManager();
        ...
        try {
            ...
            // Sub query.START
            // Thach.Le-20150211-Deleted-Start.Fix bug #1
            // Subquery sq = cq.subquery(Request.class);
            // Thach.Le-20150211-Deleted-End.Fix bug #1
            
            // Thach.Le-20150211-Moved-Start.Fix bug #1
            Root<Request> rootReq = cq.from(Request.class);
            // Thach.Le-20150211-Moved-End.Fix bug #1
            ...
            // Thach.Le-20150211-Modified-Start.Fix bug #1
...
//                                            , cb.equal(rootSub.get("createdbyUsername"), username));
            Predicate predicateOfUser = cb.or(cb.equal(rootReq.get("assigneeUsername"), username)
                    , cb.equal(rootReq.get("managerUsername"), username)
                    , cb.equal(rootReq.get("createdbyUsername"), username));
            // Thach.Le-20150211-Modified-End.Fix bug #1
            ...
            // Thach.Le-20150211-Modified-Start.Fix bug #1
            // Predicate predicate = null;
            Predicate predicate = predicateOfUser;
            // Thach.Le-20150211-Modified-End.Fix bug #1
            ...
            if (predicate != null) {
                ...
                predicate= cb.and(predicate, predicateGroupId);

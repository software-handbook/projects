...
public class ExUserJpaController extends UserJpaController {
    ...
    public ExUserJpaController(EntityManagerFactory emf) {
        super(emf);
        this.groupId = AppCons.DEFAULT_GROUP;
    }
    
    public ExUserJpaController(EntityManagerFactory emf, String groupId) {
        super(emf);
    ...
    */
    ...
    public boolean welcomeNewUser(User user) {
    ...
    * @return list of entity User
    */
    public List<User> findUserEntitiesInGroup() {
        List<User> listUser;
        EntityManager em = getEntityManager();
        try {
            Query query = em.createQuery("Select e FROM User e WHERE e.groupId = :groupId");
            query.setParameter("groupId", this.groupId);
            listUser = (List<User>) query.getResultList();

...
import mks.dms.service.PasswordService;
...
public class PasswordController extends BaseController {
    ...
    */
    ...
    @RequestMapping(value = "{groupName}/change-password", method = RequestMethod.GET)
    ...
    @RequestMapping(value = "{groupName}/changePassword", method = RequestMethod.POST)
    public ModelAndView changePassword(@ModelAttribute(AppCons.MODEL) @Validated ChangePasswordModel model, BindingResult bindingResult, Principal principal) {
        ...
        Entry entry = accountService.findUserByUid(username);
        ...
        if (entry != null) {
            if (accountService.checkPassword(username, password)) {
                ...
                changedPassOK = accountService.changePass(userDN, newpwd);
    ...
    */
    ...
    public ModelAndView resetPassword(@ModelAttribute(AppCons.MODEL) ResetPasswordModel model,
        ...
        UserEntry userEntry = accountService.findUserByEmail(email);
    ...
    */
    ...
    public ModelAndView goConfirmResetPassword(@RequestParam("email") String email, @RequestParam("key1") String key1, @RequestParam("key2") String key2) {
        ...
        PasswordService passwordService = new PasswordService(this.groupId);
        boolean validKey = passwordService.checkKey(email, decodedKey1, decodedKey2);
    ...
    */
    ...
    public ModelAndView confirmResetPassword(@ModelAttribute(AppCons.MODEL) @Validated ConfirmResetPasswordModel model, BindingResult bindingResult) {
        ...
        Entry entry = accountService.findUserByEmail(email);
        ...
        if (entry != null) {
            String userDN = entry.getDn();
            changedPassOK = accountService.changePass(userDN, newPasswd);

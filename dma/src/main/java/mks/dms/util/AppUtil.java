...
public class AppUtil {
    ...
    private static final String LDAP_CFG_PATH = "ldap-configuration.xml";
...
//    }
    ...
    * @param dn ex: uid=admin,ou=Users,dc=maxcrc,dc=com
    * @return
    */
    public static String parseGroup(String dn) {
        String group = null;

        if (dn != null) {
            String[] fields = dn.split(CHARA.COMMA);
            int len = (fields != null) ? fields.length : 0;

            // Get the second field: ou=...
            if (len > 2) {
                String groupdField = fields[1];

                fields = groupdField.split(STR_EQUAL);
                len = (fields != null) ? fields.length : 0;
                if (len == 2) {
                    group = fields[1];
                } else {
                    LOG.error("Wrong format of group in dn=" + dn);
                }
            } else {
                LOG.error("Wrong format of dn=" + dn);
            }
        } else {
            // Do nothing
        }
